...
func (e *etcdRegistry) registerNode(s *registry.Service, node *registry.Node, opts ...registry.RegisterOption) error {
	...
	if leaseID > 0 {
		...
		if _, err := e.client.KeepAliveOnce(context.TODO(), leaseID); err != nil {
			...
			log.Printf("Lease not found for %s %d", s.Name, leaseID)
	...
	if ok && v == h && !leaseNotFound {
		log.Printf("Service %s node %s unchanged skipping registration", s.Name, node.Id)
		return nil
	}
	...
	log.Printf("Registering %s id %s with lease %v and leaseID %v and ttl %v", service.Name, node.Id, lgr, lgr.ID, options.TTL)
...
func (e *etcdRegistry) Deregister(s *registry.Service, opts ...registry.DeregisterOption) error {
	...
	for _, node := range s.Nodes {
		...
		log.Printf("Deregistering %s id %s", s.Name, node.Id)
